    ...
    var body: some ReducerProtocol<State, Action> {
        Reduce<State, Action> { state, action in
            ...
            case .routeAction(_, action: .canConfirmTransportPIN(.confirm)):
                state.routes.push(.canAlreadySetup(.init(identificationInformation: state.identificationInformation)))
                return .none
            ...
            case .routeAction(_, action: .canScan(.scannedSuccessfully)):
                let setupCoordinatorState = SetupCoordinator.State(states: [
                    .root(.done(SetupDone.State()))
                ])
...
extension SetupCANCoordinator.State {
    ...
    init(oldTransportPIN: String, transportPIN: String?, pin: String, callback: CANAndChangedPINCallback, identificationInformation: IdentificationInformation?, attempt: Int, goToCanIntroScreen: Bool) {
